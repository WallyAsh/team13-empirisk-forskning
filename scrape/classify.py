...
# Available models
AVAILABLE_MODELS = ['deepseek', 'chatgpt']  # Add more as they are implemented
...
def run_model_processor(model, args):
    ...
    if not os.path.exists(processor_script):
        print(
            f"Error: Processor script for model '{model}' not found at {processor_script}")
        return 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="AllSides News Classifier - Multi-model launcher")
